...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  ...
  if(l->head == NULL){
    l->head = newNode;
      ...
      c = l->head;
      curstart = c->blk->start;
      if(c->next == NULL){
        if(newstart > curstart){//put after current
          c->next = newNode; 
          newNode->next = NULL;
          //c->next = newNode;
          ////losing original next for the cur 
        }
      }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   * 
   *    USE the compareSize()
   * bool compareSize(int a, block_t *b) if a is greater return true 
   */
  
  int blocksize = newblk->end - newblk->start+1; 
  node_t *c = l->head; 
 bool bigger =compareSize(blocksize, c->blk);
 node_t *newNode = node_alloc(newblk);
  if(bigger == false)
    newNode->next = c->next;
    c->next = newNode;
